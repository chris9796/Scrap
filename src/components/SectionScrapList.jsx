import React from "react";
import "../components_css/SectionScrapList.css"
import ScrapListLine from "./ScrapListLine ";
function SectionScrapList(){

    return(
        <ul className="SectionScrapList">
            <ScrapListLine summary="넌 첫번째야 "/>
            <ScrapListLine summary="넌 2번째야 "/>
            <ScrapListLine summary="넌 3번째야 "/>
            <ScrapListLine summary="넌 4번째야 "/>
            <ScrapListLine summary="넌 5번째야 "/>
            <ScrapListLine summary="넌 6번째야 "/>
            <ScrapListLine summary="넌 7번째야 "/>
            <ScrapListLine summary="넌 8번째야 "/>
            <ScrapListLine summary="넌 9번째야 "/>
            <ScrapListLine summary="넌 10번째야 "/>
            
        </ul>
    );
}

export default SectionScrapList;